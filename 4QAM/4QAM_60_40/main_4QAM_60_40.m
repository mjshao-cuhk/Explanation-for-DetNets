...
% settings: 4QAM,  60 by 40 MIMO (real-sized)
% last updated on Jan. 30, 2023
% If you have any questions, please contact {mjshao,liujunbin}@link.cuhk.edu.hk

%%
%rng(0);
M = 30; N = 20; % N - number transmit symbols, M - receive dimension.
QAM_size = 4;     %QAM size
mod_ord = log2(QAM_size);
SNR = 0:2:20;   %SNR in dB
max_no_simulations = 10; % No. of Monte-carlo simulations used
...
for snr_pt = 1 : (length(SNR)) 
    for simulation_no = 1 : max_no_simulations
        ...
        %sc_NI = s_NI(1:N)+sqrt(-1)*s_NI(N+1:2*N);
