...
class ADMM(nn.Module):
     def __init__(self, cons, M, N, L, batch_size):
          ...
          #self.tau = nn.Parameter( torch.ones([self.N, self.L])*350 )
          ...
          temp = []
          for ii in range(self.L):
               temp.append(0.495-(0.495-0.001)/self.L*ii)
     ...
     def forward(self, x_ini, y, H, xt):
          ...
          for ii in range(self.L):
              ...
              z = torch.tanh(temp/self.gamma[ii]*0.5)
...
for epoch in range(num_iter):  # num of  train iter
     ...
     if epoch % 5 == 0:
          s_ADMM_unfold = funcs.de_QAM(x_est, constellation, 1, N).numpy().reshape(N, -1)
          s_ADMM_unfold = s_ADMM_unfold.reshape(batch_size, N, 1)
          sc_ADMM_unfold = s_ADMM_unfold[:, 0:int(N / 2), :] + 1j * s_ADMM_unfold[:, int(N / 2):int(N), :]
          BR_count = funcs.BR(sc_ADMM_unfold, X_c)
