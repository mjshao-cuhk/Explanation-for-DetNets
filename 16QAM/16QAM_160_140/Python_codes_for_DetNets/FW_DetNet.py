...
class FW_unfold(nn.Module):
     def __init__(self, cons, M, N, L):
          ...
          for ii in range(self.L):
               temp.append(0.015-(0.015-0.005)/self.L*ii)
...
for epoch in range(num_iter):  # num of  train iter
     ...
     if epoch % 5 == 0:
          s_UPGD = funcs.de_QAM(x_est, constellation, 1, N).numpy().reshape(N, -1)
          s_UPGD = s_UPGD.reshape(batch_size, N, 1)
          sc_UPGD = s_UPGD[:, 0:int(N / 2), :] + 1j * s_UPGD[:, int(N / 2):int(N), :]
          BR_count = funcs.BR(sc_UPGD, X_c)
